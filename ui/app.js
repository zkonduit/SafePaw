...
// State Management Layer - Handles all data fetching and state
...
// Presentation Layer - Handles rendering only
class SafePawVillage {
    ...
    setupDragControls() {
        ...
        this.world.interactiveChildren = false; // Prevent children from blocking events
    ...
    createHouse(vm, x, y) {
        ...
        const totalResources = memoryGB + diskGB * 0.1; // Weigh memory more than disk

        // House size scales with total resources
        // Small VMs: 7x7
        // Medium VMs: 10x10
        // Large VMs: 13x13
        let houseSize;
        ...
        console.log('Creating house:', {
            vm: vm.name,
            resources: { memoryGB: memoryGB.toFixed(2), diskGB: diskGB.toFixed(2), total: totalResources.toFixed(2) },
        ...
        // Scale based on resources (visual metaphor) - reuse already calculated values
        const baseScale = 1.5;
        const resourceScale = 1 + Math.min(totalResources * 0.1, 1);
        houseContainer.scale.set(baseScale * resourceScale);

        // Subtle breathing animation (calming)
        const breathingSpeed = 0.0005;
        const breathingAmount = 0.02;
        houseContainer.userData = {
            baseScale: baseScale * resourceScale,
        ...
        const houseHeight = (houseSize * HOUSE_RENDER_TILE_SIZE * baseScale * resourceScale);
        ...
        let animals = [];
        if (vm.state === 'Running') {
            animals = this.createAnimals(vm, x, y, houseSize, HOUSE_RENDER_TILE_SIZE, baseScale * resourceScale);
        }
        ...
        return {
            ...
            houseScale: baseScale * resourceScale
        };
    }
...
window.addEventListener('load', async () => {
    ...
    // Create village with state manager
